...
class Labels extends CI_Controller {
	...
	function getconfig(){
		$a=getAuth();
		echo $this->session->userdata('bc_session');
		print_mz($a);
	}
	function getlabel(){
		$a=getLabel();
	...
	function main()
	{
        ...
        function listcol(){
            ...
            $colModel['label'] = array('Labels',180,TRUE,'left',2);
	...
	function submit(){
		...
		if($id != NULL && $id != '')
		{
			...
			$updatelabel=updLabel(substr($oldlabel, 0, -1),$data['label']);
			//print_mz($updatelabel);
			if($updatelabel['httpcode']==201 || $updatelabel['httpcode']==200){
				...
				print_mz($updatelabel);
				$this->session->set_flashdata("err_code", $updatelabel['response'][0]->VALIDATION_ERROR);
				$this->session->set_flashdata("message", $updatelabel['response'][0]->message);

			}
		}
		else
		{
			$insertlabeltobc=insertLabel($data['label']);
			//print_mz($insertlabeltobc);
			if(substr($insertlabeltobc['httpcode'],0,1)==2){
				...
				$this->session->set_flashdata("err_code", $insertlabeltobc['response'][0]->VALIDATION_ERROR);
				$this->session->set_flashdata("message", $insertlabeltobc['response'][0]->message);
